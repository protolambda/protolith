...
mixin StorageBlockMeta on BlockMeta {
  ...
  Future<Hash256> getStorageData(EthereumAddress address, Hash256 k) =>
      db.getData(MetaDataKey("storage", hash, [address.toUint8List(), k.uint8list])).then((b) => Hash256.fromTypedData(b));

  Future setStorageData(EthereumAddress address, Hash256 k, Hash256 v) =>
      db.putData(MetaDataKey("storage", hash, [address.toUint8List(), k.uint8list]), v.uint8list);
