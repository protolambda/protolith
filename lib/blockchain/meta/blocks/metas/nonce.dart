...
mixin NonceBlockMeta on BlockMeta {
  ...
  Future<int> getNonce(EthereumAddress address) =>
      db.getData(MetaDataKey("nonce", blockHash, [address.toUint8List()])).then(intFromBytes);

  Future setNonce(EthereumAddress address, int nonce) =>
      db.putData(MetaDataKey("nonce", blockHash, [address.toUint8List()]), encodeInt(nonce));
